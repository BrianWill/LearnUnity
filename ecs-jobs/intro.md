...
An **entity** is a piece of data known by a unique ID number and which can logically contain any number of **components** (not to be confused with Unity's GameObject Components). These components are struct types which can contain other value types but not memory references. A single entity cannot have multiple components of the same type. Components can include references to other entities by storing their unique ID numbers. Components are generally small, on the order of a few tens or hundreds of bytes.
